...
public class BasketController {
    ...
    public ResponseEntity<Void> addItems(@PathVariable String id, @RequestBody List<BasketItemDTO> basketItemDTOs) {
        basketService.addBasketItems(id, basketItemDTOs);
        return ResponseEntity.status(HttpStatus.OK).build();
    }

    @PostMapping("/{id}/delete-items")
    public ResponseEntity<Void> removeItems(@PathVariable String id, @RequestBody List<String> basketItemIds) {
